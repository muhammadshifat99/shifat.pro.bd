...
// the pill always opens below the trigger, so there is no side to pick
type Placement = { shift: number; tailX: number };
    ...
    <EmojiProvider data={emojiData}>
      ...
      >
        <AnimatePresence>
          {open && (
            <motion.div
              className={cn("absolute z-30 top-full mt-4", anchor)}
              initial={{
                opacity: 0,
                y: -10,
                scale: 0.85,
              ...
              exit={{ opacity: 0, y: -6, scale: 0.9, x: centering }}
              transition={
                reduced
                  ? { duration: 0.15 }
                  : { type: "spring", stiffness: 520, damping: 30 }
              }
              style={{ originY: 0, ...nudge }}
            >
              <div
                ref={placeBar}
                role="menu"
